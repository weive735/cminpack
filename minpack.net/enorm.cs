...
namespace MinpackDotNet
{
    static class Enorm
    {
        ...
        */
        ...
        const double rdwarf = 1.82691291192569e-153;
        const double rgiant = 1.34078079299426e+153;

        //__cminpack_attr__
        //real __cminpack_func__(enorm)(int n, const real *x)
        public static double enorm(int n, double[] x, int offset = 0)
        {
            //#ifdef USE_CBLAS
            ...
            else if (s2 != 0d)
            {
                if (s2 >= x3max)
                {
                    ret_val = Sqrt(s2 * (1d + (x3max / s2) * (x3max * s3)));
                }
                else
                {
