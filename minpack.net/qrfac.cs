...
namespace MinpackDotNet
{
    ...
    static class Qrfac
    {
        ...
        //	 real *acnorm, real *wa)
        internal static void qrfac(int m, int n, double[] a, int
            lda, bool pivot, int[] ipvt, ref int lipvt, double[] rdiag,
             double[] acnorm, double[] wa)
        {
            ...
            for (j = 0; j < n; ++j)
            {
                ...
                acnorm[j] = Enorm.enorm(m, a, j * lda + 0);
                rdiag[j] = acnorm[j];
                wa[j] = rdiag[j];
                if (pivot)
                {
            ...
            for (j = 0; j < minmn; ++j)
            {
                ...
                ajnorm = Enorm.enorm(m - (j + 1) + 1, a, j + j * lda);
                if (ajnorm != 0)
                {
                    if (a[j + j * lda] < 0)
                    {
                        ...
                        for (k = jp1; k < n; ++k)
                        {
                            ...
                            if (pivot && rdiag[k] != 0)
                            {
                                ...
                                if (p05 * (d1 * d1) <= epsmch)
                                {
                                    ...
                                    rdiag[k] = Enorm.enorm(m - (j + 1), a, jp1 + k * lda);
                                    wa[k] = rdiag[k];
                                }
                            }
                        }
